...
let VanishService = VanishService_1 = class VanishService {
    ...
    async getDepositAddress(tokenMint) {
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${this.apiUrl}/deposit_address`, {
    ...
    async createTrade(payload) {
        try {
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post(`${this.apiUrl}/trade/create`, finalBody, {
                headers: this.getHeaders(),
            }));
            return response.data;
        }
